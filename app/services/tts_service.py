...
def generate_simple_speech(text: str) -> bytes:
    ...
    try:
        ...
        logger.info(
            f"Generated audio data shape: {wav_data.shape if hasattr(wav_data, 'shape') else 'unknown'}"
        )
...
def generate_cloned_speech(
    text: str, prompt_wav: UploadFile, prompt_text: str
) -> bytes:
    ...
    try:
        ...
        logger.info(
            f"Generated cloned audio data shape: {wav_data.shape if hasattr(wav_data, 'shape') else 'unknown'}"
        )
