...
def chunk_audio(audio) -> list[tuple[float, float, np.ndarray]]:
    ...
    logger.info(f"Number of segments: {len(np.arange(0, total_seconds, chunk_size - overlap))}")
...
def remove_overlapping_words(all_words: List[Dict], overlap_threshold: float = 1.5) -> List[Dict]:
    """
    ...
    for i, word in enumerate(sorted_words):
        ...
        for prev_word in filtered_words[-10:]:  # Check last 10 words for efficiency
            ...
            if (time_diff <= overlap_threshold and 
                word['word'].strip().lower() == prev_word['word'].strip().lower()):
                is_duplicate = True
                break
...
def merge_segments(segments: List[Dict]) -> List[Dict]:
    ...
    for word in filtered_words:
        ...
        if (current_segment is None or 
            word['start'] - current_segment['end'] > 2.0):  # 2 second gap threshold
...
def save_transcription_to_json(transcription_data: Dict[str, Any], output_path: str) -> None:
